...
#' @export
#' 
PlotManualBars <- function(fsom, fcs, 
...
Plot2DScatters <- function(fsom, 
  ...
  if(!is.null(fsom$metaclustering)){
    metacluster <- fsom$metaclustering
    ...
    for (subset in subsets[[group]]){
      ...
      for (channelpair in channelpairs){
        ...
        p <- ggplot2::ggplot(data = df_ss, 
          ...
          ggplot2::ggtitle(paste0(group, s, " ", paste(subset, 
                                                       collapse = ", "))) +
...
#' labels, the markers expression, the file distribution if present, the 
#' relative (meta)cluster expression, the cluster per metacluster percentage 
...
FlowSOMmary <- function(fsom, plotFile = "FlowSOMmary.pdf"){
  ...
  if (filePresent){
    ...
    plotList[["p6"]] <- AddStarsPies(plotList[["p6"]], arcsDf,
                                     colorPalette = grDevices::colorRampPalette(c("white",
                                                                                  ...
                                                                                  "#FF7F00", 
                                                                                  "red", 
                                                                                  "#7F0000"))(length(filesI) + 1),
                                     showLegend = FALSE) +
      ggplot2::geom_text(ggplot2::aes(x = 0, y = -1, 
                                      label = "Expected distribution"))
  }
  
  #----Relative metacluster size----
  if (metaclustersPresent){
    freqMetaclusters <- as.data.frame(metaclusters) %>% 
      dplyr::count(.data$metaclusters) %>% 
      dplyr::mutate(percentage = .data$n / sum(.data$n) * 100) %>% 
      as.data.frame() 
    orderMetaclusters <- order(freqMetaclusters$percentage, decreasing = TRUE)
    nMetaclusters <- NMetaclusters(fsom)
    metaclustersOrdened <- factor(freqMetaclusters$metaclusters, 
                                  levels = orderMetaclusters)
    backgroundColors <- gg_color_hue(nMetaclusters)[orderMetaclusters]
    freqMetaclusters$metaclusters <- metaclustersOrdened
    plotList[["p7"]] <- ggplot2::ggplot(freqMetaclusters) +
      ggplot2::geom_bar(ggplot2::aes(x = .data$metaclusters,
                                     y = .data$percentage, 
                                     fill = .data$metaclusters), 
                        stat = "identity", col = "black") +
      ggplot2::theme_minimal() + 
      ggplot2::ggtitle("Relative metacluster size") + 
  ...
  if (metaclustersPresent){
    freqClusters <- 
      data.frame(Metaclusters = fsom$metaclustering,
                 Cluster = factor(nNodes, 
                                  levels = order(fsom$metaclustering)),
                 n = freqClusters$n,
                 Percentage = freqClusters$freq)
    fill <- freqClusters$Metaclusters
    legendtitle <- "Metaclusters"
  } else {
    freqClusters <- data.frame(Cluster = factor(nNodes),
                               n = freqClusters$n,
                               Percentage = freqClusters$freq)
    fill <- freqClusters$Cluster
    legendtitle <- "Clusters"
  }
  plotList[["p8"]] <- ggplot2::ggplot(freqClusters) +
    ggplot2::geom_bar(ggplot2::aes(x = .data$Cluster, 
                                   y = .data$Percentage, 
                                   fill = fill), 
                      stat = "identity", col = "black") +
    ...
    ggplot2::theme(panel.grid.major.x = ggplot2::element_blank(),
                   ...
                   axis.text.x = ggplot2::element_text(angle = 90, 
                                                       v = 0.5, hjust = 0)) +
    ggplot2::ggtitle("Relative Cluster size")
