...
#' @param  ppFiles         FCS files that correspond to the input of FlowSOM
#' @param  selectionFiles  Files indicating which cells of the original files
...
#'
#' @export 
SaveClustersToFCS <- function(fsom, originalFiles, 
...
#' @param  population       or Populations of interest. Default is c("clusters",
...
GetFeatures <- function(fsom, 
                        files, 
                        population = c("clusters", "metaclusters"),
                        type = "counts", 
                        MFI = NULL, 
                        filenames = NULL, 
                        silent = FALSE) {
...
#' GroupStats 
#' 
#' Calculate statistics between 2 groupes based on the \code{\link{GetFeatures}}
#' output
#' 
...
#' the fold and log10 fold changes beteween the medians of the 2 groups
#' 
#' @examples 
#' # Build FlowSom result
#'  fileName <- system.file("extdata", "68983.fcs", package = "FlowSOM")
